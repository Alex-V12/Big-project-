...
- Both have same input : network is a digraph in a networkx object examples of these are at the bottem of the script, source and sink are nodes in the network.
- Solve_single(Network,Source,Sink) output : original network max flow, single bottle neck edge with increase upper bound,(to what value the capacities can improve the max flow).   
